...
import { getATeacher, updateTeacher } from "../model/teacher/teacherModel.js";
...
router.post("/login", async (req, res, next) => {
  try {
    ...
    const user = await getATeacher({ email });
...
router.delete("/logout", auth, async (req, res, next) => {
  try {
    ...
    await updateTeacher({ email }, { refreshJWT: "" });
    // verify jwt
